...
class Bower extends Registry {
  ...
  getPackageInfo(packageName, key) {
    let infoPath = path.join(this.getRelativePackagePath(packageName), this.options.componentJson);
    if (!file.exists(infoPath)) {
      infoPath = path.join(this.getRelativePackagePath(packageName), this.options.json);
    }
