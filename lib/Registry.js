...
class Registry {
  constructor(options = {}) {
    ...
    this.relative = path.relative(process.cwd(), this.options.cwd);
  }
  ...
  getPackageFiles(packageName, options = {}) {
    return finder.listFiles(this.getRelativePackagePath(packageName), '**/*', options);
  }
  ...
  getPackageTypedFiles(packageName, options = {}) {
    ...
    let dir = this.getRelativePackagePath(packageName);
  ...
  getRelativePackagePath(packageName) {
    return path.join(this.relative, this.options.dir, packageName);
  }
}
