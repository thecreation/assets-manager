    ...
    return new Promise((resolve, reject) => {
      if(!this.hasPackage(name)) {
        if(options.ignoreError) {
          log(chalk.red(`Package ${name} is not exists.`));
          return resolve();
        }
      ...
      if(!pkg.isInstalled()) {
        if(options.ignoreError) {
          log(chalk.red(`Package ${pkg.options.registry}:${name} is not installed.`) + ` Please ${pkg.getInstallCmd()}.`);
          return resolve();
        }
      ...
      if(options.verbose) {
        log(chalk.blue('Copying Package ') + name + ':');
      }
